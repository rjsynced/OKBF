...
const ViewKeycap = ({ handleAddProduct }) => {
    const { id } = useParams();
    ...
    return (
        <div className='container grid grid-cols-2 gap-4 mt-8'>
            <div>
                <Carousel>
                    <div>
                        <img src={keycap.imgUrlMain} alt="pic" />
                    </div>
                    <div>
                        <img src={keycap.imgUrlC1} alt="c1" />
                    </div>
                    <div>
                        <img src={keycap.imgUrlC2} alt="c2" />
                    </div>
                    <div>
                        <img src={keycap.imgUrlC3} alt="c3" />
                    </div>
                    <div>
                        <img src={keycap.imgUrlC4} alt="c4" />
                    </div>
                </Carousel>
                <h1>Specs:</h1>
                <div className=''>
                    ...
                    <p>Total Keys: {keycap.totalKeys}</p>
                    <p>Weight: {keycap.weight}g</p>
                    <p>Compatibility: {keycap.compatibility}</p>
                </div>
            </div>
            <div className='flex-direction: column '>
                ...
                <div>
                    {/* <label>In Stock?</label><span>     </span><label>{keycap.quantity > 0 ? <box-icon name="hot"></box-icon> : "No"}</label><br /> */}
                    {/* <label>On Sale? {keycap.onSale ? "Yes" : "No"}</label> */}
                </div>
                <h3>
                    Price: ${keycap.price}
                </h3>
                <div className='flex'>
                    <p className='text-xl'>Just <span className='text-red-600'>{keycap.quantity}</span> in stock</p>
                </div>
                <div className='mb-4'>
                    <button onClick={() => {
                        handleAddProduct(keycap);
                        sucessNotify();
                ...
                <Container >
                    <div className='ratio ratio-16x9'>
                        <iframe className="w-full aspect-video hover:aspect-square" src={`https://www.youtube.com/embed/${keycap.ytEmbed}`}></iframe>
                    </div>
                </Container>
            </div>
